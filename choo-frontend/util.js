...
exports.getViewUserURL = function (state, user) {
  console.log("AM I LOGGED IN?", state.loggedIn)
  if (!user || !state.loggedIn) return ''
...
exports.getSearchUsersURL = (name) => {
  return USER_SERVICE_URL + '/' + PREFIX + '/' + USERS + '/' + SEARCH + '/' + name
}

exports.getSearchTweetsURL = (term) => {
  return TWEET_READER_URL + '/' + PREFIX + '/' + SEARCHES + '/' + term
}

exports.getSearchHashtagsURL = (hashtag) => {
  return TWEET_READER_URL + '/' + PREFIX + '/' + HASHTAGS + '/' + hashtag
}
